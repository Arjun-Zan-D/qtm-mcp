...
def create_server() -> FastMCP:
    ...
    async def analyze_gait_cycle(patient_id: str, session_date: str) -> list[TextContent]:
        ...
        *session_date* and returns a structured prompt asking the LLM to
        ...
        prompt_text = (
            ...
            "2. **Force Plates** — read resource `qtm://sessions/{patient_id}/{session_date}/force_plates` "
    ...
    async def summarize_clinical_session(patient_id: str, session_date: str) -> list[TextContent]:
        ...
        prompt_text = (
            ...
            "2. Read `qtm://sessions/{patient_id}/{session_date}/anthropometrics` for "
            "body-segment parameters (height, mass, leg length).\n"
            "3. Read `qtm://sessions/{patient_id}/{session_date}/force_plates` for "
            "ground reaction force summary statistics.\n"
            "4. Optionally read `qtm://sessions/{patient_id}/{session_date}/emg` for "
